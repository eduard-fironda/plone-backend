    ...
    stage('Release on tag creation') {
      ...
      steps{
          withCredentials([string(credentialsId: 'eddie-eea-jenkins-token', variable: 'GITHUB_TOKEN'),  usernamePassword(credentialsId: 'eddie-jekinsdockerhub', usernameVariable: 'DOCKERHUB_USER', passwordVariable: 'DOCKERHUB_PASS')]) {
           sh '''docker pull eduardvalentin/gitflow; docker run -i --rm --name="$BUILD_TAG"  -e GIT_BRANCH="$BRANCH_NAME" -e GIT_NAME="$GIT_NAME" -e DOCKERHUB_REPO="eduardvalentin/plone-backend" -e GIT_TOKEN="$GITHUB_TOKEN" -e DOCKERHUB_USER="$DOCKERHUB_USER" -e DOCKERHUB_PASS="$DOCKERHUB_PASS" -e DEPENDENT_DOCKERFILE_URL="eea/eea-website-backend/blob/master/Dockerfile eea/fise-backend/blob/master/Dockerfile eea/advisory-board-backend/blob/master/Dockerfile eea/bise-backend/blob/plone-6/Dockerfile" -e GITFLOW_BEHAVIOR="RUN_ON_TAG" eduardvalentin/gitflow'''
         }
